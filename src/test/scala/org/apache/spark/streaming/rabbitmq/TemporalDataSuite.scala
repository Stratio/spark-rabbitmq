...
private[rabbitmq] trait TemporalDataSuite extends RabbitMQSuite with BeforeAndAfter {
   ...
   * RabbitMQ Properties
   */
  val queueName = Try(config.getString("rabbitmq.queueName")).getOrElse("rabbitmq-queue")
  val exchangeName = Try(config.getString("rabbitmq.exchangeName")).getOrElse("rabbitmq-exchange")
  ...
  before {
    ...
    val deleteExchangeResult = consumer ? DeleteExchange(queueName)
